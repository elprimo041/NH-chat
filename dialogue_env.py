...
class DialogueEnv():
    ...
    def __init__(self):
        ...
        # da
        self.da_df = pd.read_csv(self.params.get('path_da_for_UIpred'))
    ...
    def utterance_selection_softmax(self, chg_theme, theme, prob_actions, mei_cmd=True):
        ...
        if mei_cmd:
            ...
            if chg_theme:
                return next_sysutte, command, len(next_sysutte), 'op'
            else:
                da = self.da_df[self.da_df['speech'] == next_sysutte]['dialogue_act'].values[0]
                return next_sysutte, command, len(next_sysutte), da
        else:
            return next_sysutte
