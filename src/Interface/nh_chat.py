...
class NHChat(AbstractChat):
    ...
    def __init__(self, model="200117_c099", mode="WebCamera", user="tmp_user",
        ...
        if self.text == False:
            self.openface.start(
                "{}/{}_face.csv".format(self.data_path, self.user))
        ...
        while not self.openface.is_running:
            time.sleep(0.1)
    ...
    def process(self, is_text=False):
        ...
        # 発話選択
        user_utt = self.select_utterance(sys_utt, da, is_text)

        # 状態更新
        n_state = self.env.get_next_state(
            self.user_impression, sys_utt, user_utt)
        self.state = n_state

        # 終了判定
        self.current_turn += 1
        return self.current_turn == self.turn_num  # is_end

    def select_utterance(self, sys_utt, da, is_text=False):
        ...
        if is_text:
            ...
            while not self.record.save_complete:
                time.sleep(0.1)
            time.sleep(0.1)
            ...
            # predict mono modal
            d_pred = self.predict_dialogue(
                user_utt, sys_utt, da, user_utt_start_time)
            ...
            # 値のrange変更
            self.user_impression = pf.changePredValueRange(
    ...
    def predict_text(self, user_utt):
        ...
        pca = pickle.load(
            open("../refData/RL_files/modelUIpred/pca.model", "rb"))
